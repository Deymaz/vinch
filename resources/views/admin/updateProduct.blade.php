    ...
    <div class="content">
        <form action="{{ route('updateProduct', [app()->getLocale(), $product->id]) }}" method="post"
            ...
            <div class="textarea-admin">
                ...
                <textarea class="form-control" rows="5" cols="10" name="description_ru"
                          id="description_ru">{{ $product->description_ru }}</textarea><br>
            </div>
            <div class="textarea-admin">
                <label for="description_ua">Описание на украинском</label>
                <textarea class="form-control" rows="5" cols="10" name="description_ua"
                          id="description_ua">{{ $product->description_ru }}</textarea><br>
            </div>
            <div class="textarea-admin">
                <label for="description_en">Описание на английском</labelfloat-left small-padding-right>
                <textarea class="form-control" rows="5" cols="10" name="description_en"
                          id="description_en">{{ $product->description_ru }}</textarea><br>
            </div>
